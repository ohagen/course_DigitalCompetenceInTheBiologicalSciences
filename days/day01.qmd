...
-   explain what biological data represents and what it does not
-   distinguish data lifecycles (stages) e.g. data entry, data storage, data analysis
-   recognize common sources of data inconsistency
...
Throughout this course, we work with a simplified but realistic
scenario:

> You are working with biodiversity monitoring data from a national
> protected area in Belize.\
> Data are collected by rangers, NGOs, and external consultants over
> long periods of time.

Your task is not 'necessarily' to be clever.\
Your task is to be **reliable**.
...
This is why interpretation without context is dangerous.

---

In this course, we use the following working definitions:

-   **Observation**: one recorded event or measurement\
    e.g. one species observation at one location and time
-   **Variable**: a property that can vary between observations\
    e.g. species name, location, count, date


A table is not meaningful by itself.\
Meaning comes from **structure and context**.

## Data vs observations vs measurements
...
Excel is widely used in biology because it is:
...
![Excel-induced gene name errors over
time](https://journals.plos.org/ploscompbiol/article/file?id=10.1371/journal.pcbi.1008984.g001&type=large){fig-align="center"}
