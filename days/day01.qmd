...
- What Excel is good at — and where it fails
...
-   Course is structured (5 days, Excel → R)
...
From Excel
1. Create a new sheet with the data above
2. Save as `.csv` and inspect the file in a text editor

From text editor
1. Create a new text file with the data above
...
- the same file opens differently on different machines 😖
- the same file opens differently on Windows vs macOS 🤮
...
1. Open the provided spreadsheets:

* Download and check [this supplementary material excel sheet](http://journals.plos.org/plosone/article/asset?unique&id=info:doi/10.1371/journal.pone.0145247.s002)/ hands on example on gene name errors in genomics
* Open [`csv/Caxiuana_tree_trait_data.csv`](https://github.com/ohagen/course_DigitalCompetenceInTheBiologicalSciences/blob/main/data/raw/csv/Caxiuana_tree_trait_data.csv) by double-clicking it (bad practice, but common) as well as inspecting it in a text editor (e.g. Notepad, VSCode, TextEdit).

Identify **data structure and separators**, **potential data integrity issues** as well as possible **Excel misbehavior**.

- Do not fix anything yet  
- Just write down what looks suspicious
...
3. Inspect:
   ...
   - decimal separator
   - encoding (UTF-8)
4. **Explicitly set column data types**
5. Click *Load*
...
This prevents Excel from “helping”. Try this out in any of the files.
...
::: {.callout-note}
- `View → Freeze Top Row` (column headers)
- `View → Freeze Panes` (ID columns)
...
::: {.callout-tip icon="🧪"}
### Alone: Play with PSP and GAP data files (5 min)

1. Open any of the PSP or GAP data files.
...
- **PSP_id** (unique identifier, e.g. metal tree tag)
- **survey_date** (date of measurement)
...
- prevent accidental re-sorting or overwriting

#### Metadata

Decide:
- what metadata belong in a **separate sheet**
- what metadata must exist **outside Excel** (e.g. README)

💬 **Be ready to explain**
- why each column has its format
- what mistakes your design prevents
- where Excel stops being sufficient
:::


## Additional Excel knowledge relevant to common failures

::: {.callout-warning}
Excel gives **no warning** when approaching limits in CSV workflows.

Excel has no audit trail

- changes cannot be reconstructed
- undo history is temporary
- collaborators overwrite silently
