...
::: {.callout-note}
- `View → Freeze Top Row` (column headers; German Excel: Ansicht → Fenster einfrieren → Oberste Zeile einfrieren)
- `View → Freeze Panes` (ID columns; German Excel: Ansicht → Fenster einfrieren → Fenster einfrieren)
- Conditional formatting (Home → Conditional Formatting; German Excel: Start → Bedingte Formatierung) to:
  - flag missing values
...
1. Select the column
2. Right-click → *Format Cells* (German Excel: Rechtsklick → *Zellen formatieren*)
3. Set:
   - Text for IDs
...
3. Freeze the top row.. see how it helps you navigate the data. (German Excel: Ansicht → Fenster einfrieren → Oberste Zeile einfrieren)
...
Home → Conditional Formatting (German Excel: Start → Bedingte Formatierung) → Choose any Icon Set or Color Scale (obs. this will ignore text values)

5. Right click the top and bottom cells and then Format Cells to see the current cathegory.

5. Select the entire column "geneSymbol" properties and right click → Format Cells → Text. What happens to the values in that column?
...
1. `Data → Get Data → From File → From Text/CSV` (German Excel: `Daten → Daten abrufen → Aus Datei → Aus Text/CSV`)
Alternatively, if avaiable,  you can use:
`Data → From Text/CSV` (German Excel: `Daten → Aus Text/CSV`)
...
3. Inspect:
   ...
   - decimal separator
   - encoding (UTF-8)
4. Click *Transform Data* (German: *Daten transformieren*) to open Power Query Editor for more control. Use *Load* (German: *Laden*) only if you want to proceed with Excel default data transformation
...
5. **Explicitly set column data types**
    - Select columns
    - Right-click → *Change Type* (German Power Query: *Datentyp ändern*) → choose correct type (Text, Decimal Number, Date, etc.)
    Alternatively you can change the type on the formula bar
   
6. Click *Close & Load* (German: *Schließen & Laden*) to import the data into Excel

Check the imported data. Did anything change?

**Steps Data Tools → Text to Columns**
...
4. Go to `Data → Text to Columns` (German Excel: `Daten → Text in Spalten`)
5. Choose *Delimited* (German: *Getrennt*) → Next
6. Select different delimiters and see what happens with the dataset
7. Select the correct delimiter (Comma, Tab, Semicolon) as well as types → Next (German: passenden Trenner wählen, dann *Weiter*)
What happens to the data? Check specifically for the last variable (column) of line 32. Right click it → Format Cells and see the Cathegory and compare with the cell above it.
...
2. `Data → Data Validation` (German Excel: `Daten → Datenüberprüfung`)
3. Choose:
   - numeric range
   - whole numbers only
   - allowed values
...
3. `Data → Data Validation` (German: `Daten → Datenüberprüfung`)
4. Under *Allow*, choose **List** (German: *Zulassen → Liste*)
...
`Home → Sort & Filter → Sort A to Z` (German: `Start → Sortieren und Filtern → Von A bis Z sortieren`) or `Right click → Sort → Sort Smallest to Largest` (German: `Rechtsklick → Sortieren → Kleinste bis Größte`)
...
7. Now try pasting with `Ctrl + Alt + V` and select Values or `Right click > Paste Special > Values` (German: `Rechtsklick → Inhalte einfügen → Werte`).
What happens now?
...
For reference, use [`data/templates/PSP/PSP_#####_SFBC.xlsx`](https://github.com/ohagen/course_DigitalCompetenceInTheBiologicalSciences/blob/main/data/templates/PSP/PSP_%23%23%23%23%23_SFBC.xlsx)
