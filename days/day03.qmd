---
title: "Day 3 – From repetition to algorithms"
execute:
  freeze: auto
editor:
  markdown:
...
**Programming** allows us to express repetitive or complex tasks as a clear set of instructions – an algorithm – so the computer can execute them quickly and consistently. Just as a recipe guides a cook through steps to bake a cake, an algorithm guides the computer through steps to solve a problem. Today, we’ll demystify programming by starting with simple logic and gradually building up to writing our own small functions in R. The goal isn’t to turn you into software engineers overnight, but to show how a little coding can save a lot of time and prevent errors.

-----------------------------------------------------------------------

## Agenda

**13:30–14:15 | Block 1 – Algorithms and pseudocode**  
...
**14:30–15:15 | Block 2 – Programming basics in R**  
...
**15:30–16:15 | Block 3 – Functions and tooling in R**  
...
- The R ecosystem: packages and libraries (using `install.packages` and `library` to extend R’s capabilities)  
...
**Algorithm** is a step-by-step procedure to accomplish a task. It’s like a cooking recipe: a clear sequence of instructions that leads to a desired outcome. In data analysis, an algorithm could be something like: *“For each observation, if the value is negative, mark it as an outlier; otherwise include it in the summary.”*

Writing out an algorithm forces you to be precise about what needs to be done – and in what order. Even sorting a list of numbers or calculating a summary statistic involves an algorithm (either one you write, or one built into a software tool).
...
Let’s say we want to compute the **average (mean)** of a list of numbers without using a built-in formula:
...
This is an algorithm for the mean. It’s conceptually what the formula does, but written as explicit steps. We could write it in plain English, as above, or in a structured 
```{r eval=F}
pseudocode:
...
if count is 0: 
  ...
  return mean
```
Even if you never coded before, this pseudocode is understandable. It’s essentially how you would explain the process to someone else. The computer, however, needs it in a more formal language (like R). But writing pseudocode first helps clarify your logic before worrying about R syntax.
...
- Repetition: the “for each value” part is a loop.

Another everyday example – a protocol for labeling samples:
...
Now let’s bring these ideas into R. Programming in R (or any language) requires understanding how the language represents data and what operations it provides.
...
| Range | approx ±1.8 × 10³⁰⁸ | −2,147,483,648 to 2,147,483,647 |
...
**Range ≠ precision**  
Numerics can be astronomically large or tiny, but only about **15–16 decimal digits** are reliable.  
...
- **NA:** “Not Available,” a missing value placeholder. There are `NA` versions for each type (numeric NA, character NA, etc., but usually just `NA` works and propagates appropriately).
- **NaN:** “Not a Number,” a result of undefined mathematical operations (like 0/0). It’s a type of numeric NA.
- **Inf** and **-Inf:** positive or negative infinity (like the result of 1/0 or -1/0).
- **NULL:** represents the absence of any value or object (different from NA which is an element in a vector; NULL is often used to indicate “nothing” as an object or list element).
...
- **Data frame:** a table where each column can be a different type. This is like a spreadsheet or CSV file read into R – the most common structure for datasets. Columns are vectors of equal length. For example, a data frame `df` might have one numeric column `height`, one character column `species`, etc. Each row represents an observation.
- **List:** a flexible container that can hold a collection of items of possibly different types and sizes. Lists are extremely useful in R because many complex objects (models, results) are stored as lists. You can think of a list as a general toolbox – each element can be anything (a vector, a data frame, another list, …). Create with `list()` function, e.g., `my_list <- list(name="Alice", scores=c(90,95,88), info=data.frame(age=30, dept="Biology"))`. Lists don’t require all elements to be same length or type.

For the Belize data, we’ve mainly used data frames (for tables of observations). Under the hood, a data frame is actually a list of equal-length vectors (each column is a vector).
...
Before we continue, let’s quickly test what we learned form the operators and revisit how we can read data into R directly from the internet.

```{r eval=FALSE}

url <- "https://raw.githubusercontent.com/ohagen/course_DigitalCompetenceInTheBiologicalSciences/refs/heads/main/data/raw/csv/dist_day02_sep_semicolon__dec_comma.csv"
...
```
The length(diameters) is 5, but one is NA. If we do mean(diameters), as noted, we get NA because by default R can’t compute the mean if any value is missing. We must do mean(diameters, na.rm = TRUE) to remove NAs in the calculation.
...
Here ! is “not”, so !is.na(diameters) is TRUE for entries that are not NA, and we index the vector with that logical vector to keep only the non-missing values.
This combination of vector operations and logical conditions is very powerful for data cleaning – you can pick out elements meeting certain criteria without writing an explicit loop.
...
For example, if we have two numeric vectors x and y of the same length, doing x + y will automatically add element by element (no loop needed in R code – it loops internally in C, which is faster). This is **vectorization**.
...
However, understanding loops is still important. Some tasks can’t be easily vectorized or it's more intuitive to write as a loop first.
*Writing loops in R
...
R uses == for equality check (e.g., if (x == 0) {...}) and != for not equal. Also remember = is for assignment (though <- is the preferred assignment operator in R), so use == for comparison. ! is used to negate a logical condition (e.g., if (!is.na(x)) {...}), meaning “if x is not NA”. Before runing the bellow, try to think what will happen
...
In the solution above, we also showed a more idiomatic R way: using sum(scores > 80) to count, or even table(scores>80). This highlights how R’s vector operations can simplify tasks. But it’s still important to learn loops and if logic, because not everything can be neatly vectorized, and clarity sometimes trumps cleverness.
...
Why write functions? 
- **Avoid repetition:** If you find yourself copying and pasting a block of code with minor changes, that’s a candidate for a function. Write it once, parametrize the parts that change (as arguments), then call it whenever needed. 
- **Clarity:** Good function names make code easier to read (“calculate_index(df)” is more descriptive than a dozen lines of code doing it in place).
...
Notice that if you pass a vector, e.g. fahrenheit_to_celsius(c(32, 212)), it will return 0 100 (since 212°F is 100°C). The function operates on each element, leveraging R’s vectorized arithmetic. </details> 
:::

### A brief note on packages
R’s power comes not only from the base language, but from its vast ecosystem of **packages**. A package is a collection of functions, data, and documentation that extends R’s capabilities. For example: - **dplyr:** for data manipulation (we'll use it later for cleaning). - **ggplot2:** for advanced plotting. - **readr:** faster CSV reading functions. - **sf:** for spatial data operations. - … and thousands more for every niche (genomics, image analysis, etc.).
...
We won’t dive deep into packages today, but it’s good to know they exist. In fact, R itself is a collection of packages (base R plus recommended). When you load RStudio, some packages like stats and utils load by default.
**R philosophy:** There’s a saying, *“There’s probably a package for that.”* If you find yourself needing functionality beyond basic R, chances are someone in the community has created a package. Reusing these is often better than reinventing the wheel. It’s a key skill to learn how to find and use packages (CRAN, Bioconductor, GitHub are common sources) as you progress.
...
- 🚀 **Never assume “this will never happen”**  
  It already has.  
  Usually at scale.  
...
You might be thinking, “That’s a lot to remember!” And indeed, becoming comfortable with programming takes practice. The key takeaway is that programming is about breaking problems into clear steps and knowing the tools R provides to implement those steps. With these fundamentals, you can start to tackle tasks that were tedious or impossible to do in Excel, like looping through hundreds of files or applying complex logic consistently.
Our Belize case study might involve, for example, writing a loop to go through many survey files and extract a summary from each – something that would be extremely painful to do by hand. With your new skills, such tasks are within reach.
In the next session (Day 4), we will apply programming to **clean and transform data**, pushing further into making our workflow reproducible. We’ll see how writing scripts and using version control can make our scientific work more reliable and shareable.
Don’t worry if not everything clicked immediately. You now have a mental model of what’s possible, and each time you practice, these concepts will become more natural. Use the exercises and examples as references. And remember: even experienced programmers frequently look up documentation and examples – that’s part of the process!
...
Please provide your feedback on today’s session: [Anonymous feedback](https://docs.google.com/forms/d/e/1FAIpQLScEgnmEEeYGN7Bn839tHEqWLHO2CE9VVMiUv0QC7ZWQma0Umw/viewform?usp=publish-editor)
:::
