...
1. Open one of the regionalized files in Excel (three numeric columns A, B, C, 100 rows each):
  - `data/raw/csv/dist_day02_sep_semicolon__dec_comma.csv` (German settings: semicolon separator, comma decimal) → loads cleanly if your decimal is `,`
  - `data/raw/csv/dist_day02_sep_comma__dec_dot.csv` (US/UK settings: comma separator, dot decimal) → if your regional decimal is `,`, tell Excel to keep dot as decimal 
  One way is over Power Query import
    First on Home → Use First Row as Headers
    Delete the automatic conversion, did you notices it changed the values
    Select all columns clicking the first one, then shift and then the last one.
    Right Click → Change Type → Using Locale → Data Type [Decimal] → Locale English (Europe)
  
  As we learned, the other way is to copy the data (from Notepad or Notebook++) and paste it into Excel. Then on  **Data → Text to Columns →... and at the end select Advanced** There, set decimal separator to `.`). 
...
1. Using the same dataset (pick the same file as above)
...
2. **Insert → Statistic Chart → Histogram** (German Excel: **Einfügen → Diagramme → Statistikdiagramm → Histogramm**)
...
2. **Insert → Statistic Chart → Box & Whisker** (German Excel: **Einfügen → Diagramme → Statistikdiagramm → Kastendiagramm (Box & Whisker)**)
...
1. Using the same dataset (pick the same file as above that matches your regional separators/decimals)
2. For each variable (A, B, C), create a histogram using Excel:
   - Select the column
  - **Insert → Statistic Chart → Histogram** (German Excel: **Einfügen → Diagramme → Statistikdiagramm → Histogramm**)
...
2. **Insert → Scatter → Scatter with only markers** (German Excel: **Einfügen → Punkt (X Y) → Punkte (nur Marker)**)
...
3.	Create a histogram and a boxplot.

4.	Discuss with your partner: Do the mean and median differ notably? What does that tell you about the distribution (skewness or outliers)? Did you intuition matched what you saw on the histogram?

*Switch roles (driver/navigator) halfway through.* This exercise gives you first-hand experience with R's commands. Don’t worry if you get stuck – use ?functionName (e.g. ?mean) to access R’s help, or ask an instructor for a hint. Soon we will dive deeper
:::
...
# you can also load the other variant:
