---
title: "Day 4 – Cleaning data and making work reproducible"
execute:
  freeze: auto
editor:
  markdown:
...
“**Data cleaning**” might sound mundane, but in practice it’s one of the most critical and time-consuming parts of any analysis – and it’s deeply scientific. Decisions made during cleaning (like how to handle missing values or outliers) can affect results. Thus, cleaning must be done carefully and transparently, so others (and your future self) can understand and trust the final data. Today, we shift our mindset from ad-hoc fixes to systematic, documented transformations. We’ll learn how to make our workflow reproducible – meaning anyone can follow the same steps and get the same results. This involves using scripts, organizing our work, and leveraging tools like **Git** for version control and **Quarto** for integrated reporting.
...
**13:30–14:15 | Block 1 – Data cleaning as scientific work**  
- Why cleaning is not “janitorial” – it's part of science (documenting assumptions and decisions)  
...
**14:30–15:15 | Block 2 – Quality control and documentation**  
- Writing R code to find potential errors (e.g., values outside expected range, duplicates where there shouldn’t be, mismatched categories)  
...
- Commenting your code and organizing it into sections (making it readable and explaining the “why” behind changes)  
...
**15:30–16:15 | Block 3 – Reproducible workflows and tools**  
- Project structure: setting up an RStudio Project (or dedicated working directory) with subfolders (data/, scripts/, output/)  
- Using **Git for version control**: basic concepts (repository, commit, push/pull, conflict) and why “save early, save often” with history is beneficial  
...
- Any remaining pain points in your data workflow that we haven’t addressed?  
...
Real-world data is messy. In the Belize case study, imagine we receive species observation logs from multiple rangers – one uses `"Panthera onca"` for jaguar, another just writes `"Jaguar"`, some cells are left blank, and one file uses `-999` to denote a missing value. If we naively combine these, we’ll have inconsistencies and errors that could lead to incorrect conclusions.
...
It’s not just about making the dataset “pretty” – it’s about ensuring the data accurately reflect reality (as much as possible) and are structured to facilitate analysis.
...
Suppose you have a spreadsheet where each column is a separate month’s measurements:
...
This is wide. To tidy it, we’d transform it to:
...
Now “month” is a variable. This format might have more rows, but it’s consistent and easier to feed into R’s plotting or modeling functions (e.g., you can easily plot count vs month, grouped by site).
...
To analyze or report, you may want to merge Table B into Table A so each observation row has the full species name and status. This requires a **key** (here `species_code`) that is present in both tables. When merging (or *joining* as database folks say), it’s important to ensure:
- The key is consistent (no typos, same case, etc. – cleaning might be needed here).
...
A common pitfall is when keys don’t match due to inconsistencies. For instance, if species_code in one table has “PantheraOnca” and in the other “panthera_onca” or a blank for unknown species – those won’t merge. Identifying and fixing these is part of cleaning: - Use consistent coding schemes (maybe standardize to all lowercase with underscores). - Fill in missing identifiers if possible, or decide to exclude those records.

::: {.callout-tip icon="🧪"}
### Group exercise: Spot the mess (10 min)
Consider the following snippet of a dataset (imagine it’s loaded as a data frame field_data in R):
...
•	**Inconsistent plot_id format:** "A01", "A02", "A03" vs "A-04" (extra hyphen in A-04).
•	**Elevation units:** "95m" is labeled with "m" while others are numeric or one has ".".
•	**Missing value notation:** Elevation for A-04 is ".", which is not standard (should be NA or blank).
•	**Counts:** NA vs blank vs 0 – second species count for A02 is NA (probably meaning no data), whereas others have numeric including 0 (which might mean truly zero count). We should standardize how absence vs not recorded is represented.
•	**Notes inconsistency:** A03 note has a question mark "seedlings included?" – unclear data collection issue.
•	**Data structure:** species counts are in separate columns (species_1_count, species_2_count). If the number of species is large or variable, a tidy format would have a separate table or a “species” column. In this small example it’s okay, but it hints at a possible need to pivot longer if many species.
•	**Units in data:** "95m" should be numeric 95 with units documented elsewhere (units shouldn’t be mixed into the data cell with the value).
...
This is just illustrative – real cleaning might be more involved – but notice how we can chain multiple transformations. Also, each operation is documented by the code itself. If someone wonders "how did 95m become 95?" – the code shows it.
...
By including such documentation with your dataset (as a separate .md or .txt file), you ensure that context isn’t lost. This is akin to metadata you learned about on Day 1.
...
- **Commit:** a snapshot of the state of files with a message describing the changes. E.g., "Corrected elevation units and standardized plot IDs". 
...
- If you break something in your script, you can compare to yesterday’s version and revert if needed. 
- It encourages small, documented changes (commits) – which parallels documenting your work scientifically. 
- Collaboration: multiple people can work on different parts of the project, merge contributions, review changes, etc.

Even if working solo, using Git + a remote like GitHub means you won’t lose work if your laptop dies, and you can easily share your project (or publish it as supplementary material for a paper).
For example, this course material is likely maintained in a Git repo. Every edit I (the instructor) make is tracked.

Basic Git workflow: 

1. **Initialize a repo:** git init (or use RStudio’s version control features to create a new project with Git). 
2. **Stage and commit files:** git add script1.R; git commit -m "Added cleaning script" 
3. **Repeat commits** as you reach logical milestones. 
4. **View history:** git log (or GUI in RStudio/GitHub Desktop). 
5. **Undo/branch/merge** as needed (more advanced, but incredibly useful as you get comfortable).
...
- It encourages writing about what you’re doing as you do it, which is great for reflection and for others reading it.
...
- When you’re ready to create a report or publication, use a Quarto doc to perform the final analysis and generate output, with prose explaining each result.

This course website is an example: all five days are written in Quarto, combining explanation (what you’re reading) and code (to generate any figures, etc.). It’s fully reproducible – you could run the Quarto files and regenerate the entire site.

::: {.callout-tip}
**Tip:** Start small with reproducible docs. Maybe begin by using R Markdown for a simple analysis write-up or lab report, to get used to it. Over time, you’ll see the immense benefit of having everything in one place.
:::
...
- Share your project (data+code) for others to reproduce or extend – practicing true open science.
...
Today was about instilling rigor and order into the chaotic part of data analysis. We learned that cleaning data is not just pressing a few Excel buttons – it’s a process that benefits from careful thought, and coding it in R makes it reproducible and less error-prone. We saw the tidy data framework as a guide to structuring datasets, and we practiced thinking about common data issues and how to fix them in a systematic way.
...
Tomorrow, in Day 5, we’ll take a higher-level view to discuss how the tools (Excel, R, etc.) and models we choose influence our science, and reflect on everything we’ve learned. We’ll also address any remaining questions and discuss where to go next in your data journey.

By mastering the workflow up to this point, you’ve significantly upgraded your *digital competence* as a scientist – you can wrangle data, analyze it, and ensure that your work can be trusted and extended by others.
...
- Think of a dataset you’ve worked with before: what was one issue that caused a headache, and how might you tackle it now with what you learned (tidy data, scripting, etc.)?
...
Please share your feedback for today’s session:  
