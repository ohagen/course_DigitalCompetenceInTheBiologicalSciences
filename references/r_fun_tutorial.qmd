...
Functions like `data.frame()` expect "(" and ")" to enclose arguments. If you opoen a parenthesis, R expects you to close it. That's why you see the + sign in the console until you close it.
...
2.  Create a data frame called 'trees' with two columns: `tree_id` and `height_cm` (4 rows).
...
2. **Random sample**: Use `sample()` to pick 2 random trees from `trees$tree_id`.
